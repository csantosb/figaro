...
@jit
def propose_point_1d(m_o, s_o, dm, ds):
    m = m_o + (np.random.rand() - 0.5)*2*dm
    s = s_o + (np.random.rand() - 0.5)*2*ds
    return [m,s]
...
def sample_point_1d(means, covs, log_w, m_min = -20, m_max = 20, s_min = 0, s_max = 1, burnin = 1000, dm = 1, ds = 0.05, a = 2, b = 0.2):
    ...
    for i in range(burnin):
        new_point = propose_point_1d(old_point[0], old_point[1], dm, ds)
...
@jit
def MC_predictive_1d(events, n_samps = 1000, m_min = -5, m_max = 5, a = 2, b = 0.2**2):
...
def MC_predictive(events, dim, n_samps = 1000, m_min = -5, m_max = 5, a = 2, b = np.array([0.2])):
    ...
    for i in prange(len(events)):
        logP += log_prob_mixture_MC(means, variances, events[i].log_w, events[i].means, events[i].covs)
    return logsumexp(logP) - np.log(n_samps)
